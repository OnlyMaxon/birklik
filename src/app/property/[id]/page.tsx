...
// Несуществующее объявление отдаёт 200, а не 404, и это осознанная плата за
// потоковую отдачу: без неё воркер не укладывается в 10 мс процессорного
// времени тарифа Workers Free — подробности в `src/app/loading.tsx`.
//
// Механика честных 404 готова и проверена (`layout.tsx` рядом считает
// существование до границы Suspense) — включается удалением корневого
// loading.tsx после перехода на платный тариф.
//
// Пока статусом не отбиться, отбиваемся мета-тегом: noindex и БЕЗ canonical.
// Canonical на несуществующий адрес был прямым приглашением его
// проиндексировать.
