...
export const ReportCommentModal: React.FC<ReportCommentModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await reportCommentAction(propertyId, commentId, commentText, reason, details)

      if (!result.success) {
        setMessage({
          type: 'error',
          text: result.error === 'duplicate'
            ? (language === 'en'
              ? 'You have already reported this comment'
              : language === 'ru'
                ? 'Вы уже пожаловались на этот комментарий'
                : 'Siz artıq bu şərh haqqında şikayyət verdinisiniz')
            : (language === 'en' ? 'Error submitting report' : language === 'ru' ? 'Ошибка при отправке жалобы' : 'Şikayyət göndərərkən xəta')
        })
